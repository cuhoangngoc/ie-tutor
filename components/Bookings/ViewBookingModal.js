...
const ViewBookingModal = ({ booking }) => {
  return (
    <>
      <button
        type="button"
        class="inline-flex items-center justify-center gap-2 rounded-md border border-transparent bg-blue-500 px-4 py-2 text-sm font-semibold text-white transition-all hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        data-hs-overlay={`#modal-${booking.id}`}
      >
        Details
      </button>
      ...
      >
        <div class="mt-10 w-full max-w-full opacity-0 transition-all hs-overlay-open:mt-0 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 md:mx-auto md:mt-0 md:max-w-lg md:hs-overlay-open:mt-10">
          <div class="flex flex-col bg-white dark:bg-gray-800 md:rounded-xl md:border md:shadow-sm md:dark:border-gray-700">
            <div class="flex items-center justify-between border-b px-4 py-3 dark:border-gray-700">
              <h3 class="font-bold text-gray-800 dark:text-white">Booking details</h3>
              <button
                type="button"
            ...
            <div class="overflow-y-auto p-4">
              {Object.entries(booking).map(([key, value], i) => {
                return (
                  <div class="flex justify-between py-2" key={i}>
                    <span class="text-gray-700 dark:text-gray-400">
                      {convertCamelCaseToReadable(key)}
                    </span>
                    {key === 'createdAt' || key === 'startDate' || key === 'endDate' ? (
                      <span class="text-gray-700 dark:text-gray-400">
