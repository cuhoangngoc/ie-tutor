...
const Card_info = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/subjects/all`);
      const data = res.data;
      setData(data);
    };
    fetchData();
        ...
        >
          ...
          <div className="col-span-2 w-full flex-col justify-between">
            ...
            {/* <div className="mt-2  flex flex-col gap-2 md:flex-row ">
              <p className="self-start font-bold md:self-center">Availability</p>
            ...
            <div className="mt-2 ">
              <p className="font-bold">You can get teaching service direct at</p>
              <ul className="mt flex flex-row flex-wrap gap-2">
                ...
                <li className=" flex flex-row items-center gap-2 rounded-lg bg-gray-200 px-4 py-2">
                  <SlCamrecorder className="text-red-400" /> <span>Offline</span>
                </li>
                <li className=" flex flex-row items-center gap-2 rounded-lg bg-gray-200 px-4 py-2">
                  <SlCamrecorder className="text-red-400" /> <span>Student place</span>
                </li>
                <li className=" flex flex-row items-center gap-2 rounded-lg bg-gray-200 px-4 py-2">
                  <SlCamrecorder className="text-red-400" /> <span>Tutor place</span>
                </li>
              </ul>
            </div>
             */}
            <div className="mt-4 max-h-20 overflow-y-auto">
              <p dangerouslySetInnerHTML={{ __html: data.instructorBio }}></p>
            </div>
